...
knitr::include_graphics("./230_Isserlin_RCy3_intro/images/screenshot_availablenodecolumns.png")
```
...
Using the Cytoscape App, aMatReader[@amatreader], we transform our adjacency matrix into an interaction network. First we filter the corrleation matrix to contain only the strongest connections (for example, only correlations greater than 0.9). 
...
correlation_network_png_file_name <- paste(getwd(),"/230_Isserlin_RCy3_intro/images/","correlation_network.png",sep="")
...
```{r correlation_network2, echo=FALSE, fig.cap="Example correlation network created using aMatReader"}
...
Run gProfiler.  g:Profiler will return a set of pathways and functions that are found to be enriched in our query set of genes.  
...
```{r eval=evalcytoscape}
 ...
 em_results_filename <-paste(getwd(),
                            "230_Isserlin_RCy3_intro/data",paste("gprofiler_cluster",current_cluster,"enr_results.txt",sep="_"),sep="/")
...
knitr::include_graphics("./230_Isserlin_RCy3_intro/images/screenshot_gprofiler_emresults.png")
```


Create an enrichment map with the returned g:Profiler results.  An enrichment map is a different sort of network.  Instead of nodes representing genes, nodes represent pathways or functions.  Edges between these pathways or functions represent shared genes or pathway crosstalk.  An enrichment map is a way to visualize yoru enrichment results to help reduce redundancy and uncover main themes.  Pathways can also be explored in detail using the features available through the App in Cytoscape. 
```{r eval=evalcytoscape}
...
cluster1em_png_file_name <- paste(getwd(),"/230_Isserlin_RCy3_intro/images/","cluster1em.png",sep="")
...
```{r cluster1em_fig, echo=FALSE, fig.cap="Example Enrichment Map created when running an enrichment analysis using g:Profiler with the genes that are part of cluster 1"}
...
```{r cluster1em_annot, include=TRUE}
cluster1em_annot_png_file_name <- paste(getwd(),"/230_Isserlin_RCy3_intro/images/","cluster1em_annot.png",sep="")
...
Download the latest pathway definition file from the [Baderlab download site](http://download.baderlab.org/EM_Genesets/current_release/).  Baderlab genesets are updated on a monthly basis. Detailed information about the sources cav be found [here](http://baderlab.org/GeneSets).
...
dest_gmt_file <- paste(getwd(),paste("230_Isserlin_RCy3_intro/data",gmt_file,sep="/") ,sep="/")
...
sumexpr_filename <- paste(getwd(), "230_Isserlin_RCy3_intro/data/SummarizeExperiment_expression.txt", sep="/") 
...
sumexpr_rdat_filename <- paste(getwd(), "230_Isserlin_RCy3_intro/data/SummarizeExperiment_rdat.txt", sep="/") 
...
classDefinitions_RNASeq <- read.table( 
  paste(getwd(), "230_Isserlin_RCy3_intro/data","RNASeq_classdefinitions.txt", sep="/"), header = TRUE, sep = "\t", quote="\"", stringsAsFactors = FALSE)
...
sumexpr_cdat_filename <- paste(getwd(), "230_Isserlin_RCy3_intro/data/SummarizeExperiment_cdat.txt", sep="/") 
...
Run basic Over representatin analysis (ORA) using our ranked genes and our gene set file downloaded from the Baderlab genesets.
...
knitr::include_graphics("./230_Isserlin_RCy3_intro/images/screenshot_ora_results.png")
```
...
ora.all$res.tbl <- cbind(ora.all$res.tbl, Q.VALUE = p.adjust(ora.all$res.tbl$P.VALUE, "BH"))
...
em_results_mesen <- data.frame(name = ora.all$res.tbl$GENE.SET,descr = ora.all$res.tbl$GENE.SET, 
                               pvalue=ora.all$res.tbl$P.VALUE, qvalue=ora.all$res.tbl$Q.VALUE.X, stringsAsFactors = FALSE)

#write out the ora results
 em_results_mesen_filename <-paste(getwd(),
                            "230_Isserlin_RCy3_intro/data","mesen_ora_enr_results.txt",sep="/")
...
mesenem_png_file_name <- paste(getwd(),"/230_Isserlin_RCy3_intro/images/","mesenem.png",sep="")
...
```{r mesenem_fig, echo=FALSE, fig.cap="Example Enrichment Map created when running an enrichment analysis using EnrichmentBrowser ORA with the genes differential in Mesenchymal OV"}
...
Export image of resulting Annotated Enrichment map. (Manually zoomed into a small section of the network)

```{r mesenem_annot, include=TRUE}
mesenem_annot_png_file_name <- paste(getwd(),"/230_Isserlin_RCy3_intro/images/","mesenem_annot.png",sep="")

```

```{r eval=evalcytoscape}
if(file.exists(mesenem_annot_png_file_name)){
  #cytoscape hangs waiting for user response if file alreayd exists.  Remove it first
  file.remove(mesenem_annot_png_file_name)
  } 

#export the network
exportImage(mesenem_annot_png_file_name, type = "png")
```

```{r cmesenem_annot_fig, echo=FALSE, fig.cap="Example Annotated Enrichment Map created when running an enrichment analysis using EnrichmentBrowser ORA with the genes that differential in mesenchymal OV"}
knitr::include_graphics(mesenem_annot_png_file_name)
```
