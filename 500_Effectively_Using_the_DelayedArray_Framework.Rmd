...
**TODO**: Make these types stand/boxed out

**TIP**
We can use tools from the **rhdf5** package to help us construct an *HDF5Array* from an existing HDF5 file.
Here, we use `rhdf5::h5ls()` to list the contents of an HDF5 file and wrap one of the datasets in a *DelayedArray*.
...
rhdf5::h5ls(tally_file)
HDF5Array(filepath = tally_file, name = "/ExampleStudy/16/Counts")
```
...
Not suprisingly, working with in-memory seeds will typically be faster; you are trading off increased memory-usage for faster performance.
