...
- Packages for performing matrix factorizations, (generalized) linear regression, k-nearest neighbours, etc. (work in progress)
...
The seed contract is fairly minimal^[See `vignette("02-Implementing_a_backend", "DelayedArray")` for details.]:
...
**TODO:** (UP TO HERE) Minimal introduction to *DelayedOps*; the details aren't so important and we'll talk more about the practical side of the *DelayedOps* subclasses when describing delayed operations.

#### So what seed should I use?

Notably, `da_mat`, `da_Mat`, `da_tbl`, `da_df`, `da_Rle`, and `da_hdf5` now all "look" much the same, and we'll soon see that they "feel" much the same, too.
