...
For this analysis, we will call peaks from islands of coverage scores
over chromosome 10.
...
```{r}
chr10_scores_by_signal <- chr10_scores %>% group_by(signal_type)
```

We can then filter to find the coordinates of the peak containing the maximum
score for each signal.
We can then find a 5000 nt region centered around the maximum position 
...
```{r peak_region}
peak_region <- chr10_scores_by_signal %>%
...
3. Plot the maximum scores for each signal type using the `ggbio` package.
