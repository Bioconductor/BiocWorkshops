...
This section summarizes three fundamental data classes for the representation of
...
(`rowData`), column/sample phenotype or clinical data (`colData`),
and the overall experiment (`metadata`). The derivative class
...
`SummarizedExperiment` and `RangedSummarizedExperiment`, so everything you learn
about `SummarizedExperiment` will be applicable to `SingleCellExperiment`.
...
methods adopt the same vocabulary as `SummarizedExperiment`.
...
Another advantage of `RaggedExperiment` is that it provides a flexible set of
_*Assay_ methods to support transformation of data to matrix format.
The following schematic shows some possible data transformation operations
available for `RaggedExperiment` objects.
...
The `RaggedExperiment` class is similar to the `SummarizedExperiment` class
in that it supports multiple assays on a number of features (based on the number
of `mcols` in the originating `GRangesList`). It also supports most operations
available for `SummarizedExperiment` such as `rowRanges`, `assay`, and
`subsetByOverlaps`.

Generally in a `SummarizedExperiment`, all feature measurements are consistent
across all samples. One of the advantages of using a `RaggedExperiment` over a
`SummarizedExperiment` object is that "ragged" or sparse features can be
...
The `MultiAssayExperiment` class is designed to handle many Bioconductor classes
with a minimum set of requirements. These method requirements are: `dimnames`,
bracket subsetting (`x[rows, columns]`), and `dim`.

It is modeled after the `SummarizedExperiment` class. As with the
`RaggedExperiment` class, `MultiAssayExperiment` shares many methods with
`SummarizedExperiment`. Below is a table of methods that are in common accross
these classes.

| Method</br>function | `SummarizedExperiment` | `MultiAssayExperiment` | `RaggedExperiment` |
|-----------------|:----------------------:|:----------------------:|:------------------:|
| assay(s)        | X | X | X |
| rowRanges       | X |  | X |
| subsetByOverlaps| X | * | X |
| colData         | X | X | X |

*Indirect support, use of `GRanges` for subsetting
